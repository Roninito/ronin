 ...
 * Never drops the latest user or system messages.
 */
...
export function createTokenGuardMiddleware(
  options: { maxTokens?: number } = {}
): Middleware<ChainContext> {
  return async (ctx, next) => {
    // Ensure budget is set if maxTokens option provided
    if (options.maxTokens && !ctx.budget) {
      ctx.budget = {
    ...
    budget.current = totalTokens(ctx.messages);
    await next();
  };
}
