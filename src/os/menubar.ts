...
function generateMenubarScript(port: number, routes: MenubarRoute[] = []): string {
  const routeItems = routes.map(r => {
    ...
    return `		menu item "${title}" of routesMenu`
  }).join('\n');

  const routeClickHandlers = routes.map((r, idx) => {
    const url = `http://localhost:${port}${r.path}`;
    return `		if the title of the clicked item is "${r.title.replace(/"/g, '\\"')}" then
			do shell script "open '${url}'"
		end if`;
  }).join('\n');

  return `-- Ronin Menubar
-- Generated by Ronin Desktop Mode
...
on run
	...
	tell application "System Events"
		tell application process "SystemUIServer"
			set statusItem to make new status item at end of status items
...
on buildMenu()
	...
	-- Full implementation would use ObjC bridge
	-- Routes submenu would be added here
end buildMenu

on updateMenu()
	-- Update status periodically
	try
		do shell script "curl -s http://localhost:${port}/api/os-bridge/status"
	on error
		-- Bridge not running
	end try
end updateMenu

on handleRouteClick(clickedItem)
${routeClickHandlers}
end handleRouteClick
 ...
 * Start menubar app
 */
export function startMenubar(port: number = 17341): boolean {
  try {
    if (isMenubarRunning()) {
    ...
    // Write menubar script
    const script = generateMenubarScript(port);
