...
export function install(options: InstallerOptions = {}): boolean {
  ...
  console.log("🔧 Installing Ronin Desktop Mode for macOS...\n");
  ...
  try {
    ...
    if (existsSync(workflowDir)) {
      console.log("  ⚠️  Quick Action already exists, removing...");
      execSync(`rm -rf "${workflowDir}"`);
    }
    
    console.log("  📦 Creating Quick Action workflow...");
    ...
    if (existsSync(plistPath)) {
      console.log("  ⚠️  LaunchAgent already exists, removing...");
      execSync(`launchctl unload "${plistPath}" 2>/dev/null || true`);
      unlinkSync(plistPath);
    }
    
    console.log("  📦 Creating LaunchAgent...");
    ...
    console.log("\n📋 What was installed:");
    console.log(`   • Quick Action: Right-click any file → Services → Send to Ronin`);
    console.log(`   • LaunchAgent: Ronin will auto-start with macOS`);
    ...
    console.log("   3. Right-click a file → Services → Send to Ronin");
    
    // 4. Install RoninTray for system tray
    console.log("\n📦 Installing RoninTray system tray application...");
    const trayInstalled = installRoninTray();
    
    if (!trayInstalled) {
      console.log("⚠️  RoninTray installation failed, but Desktop Mode is still functional");
    }
...
export function uninstall(): boolean {
  console.log("🗑️  Uninstalling Ronin Desktop Mode...\n");
  
  try {
    // 1. Remove Quick Action
    const workflowDir = join(homedir(), "Library", "Services", "Send to Ronin.workflow");
    if (existsSync(workflowDir)) {
      console.log("  🗑️  Removing Quick Action...");
      execSync(`rm -rf "${workflowDir}"`);
    } else {
      console.log("  ℹ️  Quick Action not found");
    }
    
    // 2. Remove LaunchAgent
    const plistPath = join(homedir(), "Library", "LaunchAgents", "ai.ronin.desktop.plist");
    if (existsSync(plistPath)) {
      console.log("  🗑️  Removing LaunchAgent...");
      execSync(`launchctl unload "${plistPath}" 2>/dev/null || true`);
      unlinkSync(plistPath);
    } else {
      console.log("  ℹ️  LaunchAgent not found");
    }
...
export function verify(): boolean {
  ...
  if (status.quickActionInstalled) {
    console.log("  ✅ Quick Action installed");
  } else {
    console.log("  ❌ Quick Action not found");
    allGood = false;
  }
  ...
  if (status.launchAgentInstalled) {
    ...
    try {
      ...
      if (result.includes("ai.ronin.desktop")) {
        ...
        console.log("  ⚠️  LaunchAgent installed but not loaded");
      }
    } catch {
      console.log("  ⚠️  LaunchAgent installed but not loaded");
    }
...
export function installRoninTray(): boolean {
  try {
    ...
    if (existsSync(appPath)) {
      ...
      throw new Error("RoninTray.app not found after build");
    }
