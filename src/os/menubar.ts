 ...
 */
/**
 * Generate AppleScript for menubar app.
 * Creates a simple persistent process that keeps running as a status indicator.
 * The actual menubar integration happens via native implementation.
 */
function generateMenubarScript(port: number, routes: MenubarRoute[] = []): string {
  // Simple AppleScript that just keeps the process running
  // Real menubar integration handled by system/native code
  return `-- Ronin Menubar Status Indicator
-- Runs as a persistent background process

tell application "System Events"
  -- Keep this process alive as a status indicator
  repeat
    delay 3600
  end repeat
end tell`;
...
export function showMenu(state: MenubarState): void {
  const menuItems = [
    ...
    `📝 Recent Texts (${state.recentTexts})`,
    "---",
    state.enabled ? "⏸️ Disable Desktop Mode" : "▶️ Enable Desktop Mode",
    state.offlineMode ? "🌐 Disable Offline Mode" : "📴 Enable Offline Mode",
    state.clipboardEnabled ? "📋 Disable Clipboard" : "📋 Enable Clipboard",
    "🔄 Switch AI Provider",
    "---",
    "🌐 Open Dashboard",
    state.lastSync ? `🕐 Last Sync: ${state.lastSync.toLocaleTimeString()}` : "🕐 Not synced",
    "---",
    "❌ Quit Ronin Menubar",
  ];
  ...
  const script = `
    set menuItems to {${menuItems
      ...
      .join(", ")}}
    set selectedItem to choose from list menuItems with title "🥷 Ronin Desktop" with prompt "Select an action:"
    return selectedItem
  `;
...
function showAIProviderMenu(state: MenubarState): void {
  const providers = [
    state.aiProvider === "local" ? "● 🖥️ Local (Ollama)" : "○ 🖥️ Local (Ollama)",
    state.aiProvider === "grok" ? "● ☁️ Grok" : "○ ☁️ Grok",
    state.aiProvider === "gemini" ? "● ☁️ Gemini" : "○ ☁️ Gemini",
    "---",
    "↩️ Back",
  ];
  ...
  const script = `
    ...
    set selectedItem to choose from list menuItems with title "🤖 Select AI Provider" with prompt "Choose AI provider:"
    return selectedItem
  `;
