          ...
          if (toolCallResponse.toolCalls && toolCallResponse.toolCalls.length > 0) {
            ...
            for (const toolCall of toolCallResponse.toolCalls) {
              try {
                ...
                // Check if it's a plugin tool (format: pluginName_methodName)
                if (toolName.includes("_")) {
                  const [pluginName, methodName] = toolName.split("_");
                ...
                toolResults.push({ tool: toolCall.function.name, result: `Error: ${error}` });
              }
            }
